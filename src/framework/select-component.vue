<template>
    <div class="select">
        <div class="selected-value" ref="selected-value" @click="displayOptions">{{value}}</div>
        <div class="select-options-container" ref="select-options-container" :class="{'visible': showOptions}">
            <div class="select-option" @click="select(option)" v-for="option in options" :key="option">{{option}}</div>
        </div>
    </div>
</template>
