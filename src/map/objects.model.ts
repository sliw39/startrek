...
export class Distance extends PhysicalQuantity<DistanceUnit> {
  ...
  static sunRadius(factor = 1) {
    return new Distance(0.7 * factor, "mkm");
  }
...
export class Temperature extends PhysicalQuantity<TemperatureUnit> {
  ...
  static k(value: number) {
    return new Temperature(value, "K");
  }
  static c(value: number) {
    return new Temperature(value, "C");
  }
  static f(value: number) {
...
export class PhysicalProperties {
  static parse(data: any) {
    return new PhysicalProperties(
      Distance.parse(data.radius),
      Mass.parse(data.mass),
  ...
  constructor(
    public radius = Distance.zero("km"),
    public mass = Mass.zero("earth"),
    public temperature = Temperature.zero()
  ) {}
}
...
export class OrbitalProperties {
  static parse(data: any) {
    ...
    return new OrbitalProperties(
      Distance.parse(data.semiMajorAxis) || Distance.zero("ua"),
      data.excentricity,
      Time.parse(data.period) || Time.zero("d"),
      data.tilt
    );
  }
  serialize() {
...
export class System {
  ...
  constructor(
    ...
    public objects: CelestialObject[]
  ) {
  ...
  static defaultSystem() {
    return new System(["New Sys"], coord(Point.origin()), [
      new Star(["Main Star"]),
    ]);
  }
}
...
export class CelestialObject {
  ...
  constructor(
    public names: string[],
    public orbital = OrbitalProperties.origin(),
...
export class Star extends CelestialObject {
  constructor(
    names: string[],
    public physical = new PhysicalProperties(
      Distance.sunRadius(),
      Mass.sun(),
      Temperature.sun()
    ),
    appearance = new Appearance(1),
    orbital = OrbitalProperties.origin()
  ) {
...
export class Planet extends CelestialObject {
  constructor(
    names: string[],
    orbital: OrbitalProperties | undefined,
    public physical: PhysicalProperties,
...
export function parseCelestial(data: any) {
  ...
  switch (data._type) {
    case "Star":
      c = new Star(
        data.names || [],
        PhysicalProperties.parse(data.physical),
        Appearance.parse(data.appearance),
        OrbitalProperties.parse(data.orbital)
      );
      break;
    case "Planet":
      c = new Planet(
        ...
        OrbitalProperties.parse(data.orbital),
        PhysicalProperties.parse(data.physical),
        Appearance.parse(data.appearance) || new Appearance(1)
      );
      break;
    case "Moon":
      c = new Moon(
