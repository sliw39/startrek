...
export namespace SystemIo {
  ...
  export async function listAll(deep = true) {
    ...
    return (await readSnapshots(systems, deep)).filter(
      (e) => e !== undefined
    ) as System[];
  }
  ...
  export async function find(query: string, deep = false) {
    let systems = await db.collection("system").get();
    return (await readSnapshots(systems, deep, query)).filter(
      (e) => e !== undefined
    ) as System[];
  }

  export async function findByDirectChild(childUid: string) {
    let systems = await db
      .collection("system")
      .where("objects", "array-contains", childUid)
      .get();
    return (await readSnapshots(systems, false)).filter(
      (e) => e !== undefined
    )[0];
  }

  async function readSnapshots(
    result: firebase.default.firestore.QuerySnapshot,
    deep: boolean,
...
export namespace CelestialIo {
  export async function upsert(celestial: CelestialObject) {
    if (celestial._uid) {
      await db
        .collection("celestrial")
        .doc(celestial._uid)
        .update(celestial.serialize());
    } else {
  ...
  export async function find(query: string) {
    return (await db.collection("celestrial").get()).docs
      .filter((d) => d.exists && arrayContains(d.data().names, query))
      .map((doc) => parseCelestial(mapUid(doc)));
  }
  ...
  export async function getChildren(parentUid: string) {
    ...
    return (
      await db
        .collection("celestrial")
        .where("_parent", "==", parentUid)
        .orderBy("orbital.semiMajorAxis")
        .get()
    ).docs.map((doc) => parseCelestial(mapUid(doc)));
  }
