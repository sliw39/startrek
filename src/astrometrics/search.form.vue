...
<div class="astrometrics-search">
    ...
    <div class="results">
        <div class="result" @click="select(result)" v-for="result in results" :key="result.id">
...
export default class FormCelestrial extends Vue {
    ...
    async search() {
        if(this.data.length > 2) {
            let [ systems, celestrials ] = await Promise.all(
                [SystemIo.find(this.data, false),
                CelestrialIo.find(this.data)]
            );
    ...
    async onValueChange(newval: string) {
        ...
        let cel = await CelestrialIo.get(newval);
        if(cel) {
